...
public:
    ...
    {
        //unsigned int division is faster than int division
        return (( _vector[(unsigned int)i/INTSIZE] << ((unsigned int)i%INTSIZE) ) < 0); //two's complement check of first bit
    };
    ...
    {
        _vector[i/INTSIZE] ^= (MININT >> (i%INTSIZE));
    };
